...
namespace PedeRoca.Repositories.ADO.SQLServer
{
    public class CarrinhoDAO
    {
        ...
        //------------- Details Carrinho por ID --------------- Testar
        #region "Listar Carrinho por id"
        public Carrinho DetailsCarrinhoID(int id)
        {
            Carrinho carrinho = new Carrinho();
            using (SqlConnection connection = new SqlConnection(this.connectionString))
            {
                ...
                {
                    command.Connection = connection;
                    command.CommandText = "select id_carrinhoCompra, id_usuario, id_itemCarrinho, valorTotal FROM tb_carrinhoCompras where id_carrinhoCompra=@id;";
                    command.Parameters.Add(new SqlParameter("@id", System.Data.SqlDbType.Int)).Value = id;
        ...
        public void AdicionarCarrinho(Carrinho carrinho)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                {
                    command.Connection = connection;
                    command.CommandText = "INSERT INTO tb_carrinhoCompras (id_usuario, id_itemCarrinho, valorTotal) VALUES (@id_usuario, @id_itemCarrinho, @valorTotal); select convert(int,@@identity) as id;;";

                    command.Parameters.Add(new SqlParameter("@id_usuario", System.Data.SqlDbType.Int)).Value = carrinho.pessoa.Id_usuario;
                    command.Parameters.Add(new SqlParameter("@id_itemCarrinho", System.Data.SqlDbType.Int)).Value = DetailsItemID(carrinho.pessoa.Id_usuario);
                    command.Parameters.Add(new SqlParameter("@valorSubTotal", System.Data.SqlDbType.Decimal)).Value = (DetailsItemID(carrinho.pessoa.Id_usuario).Quantidade) * (DetailsProdutoID(carrinho.pessoa.Id_usuario).PrecoUnitario);
        ...
        //------------- Remover Carrinho----------------------- Testar
        #region "Remover carrinho"

        public void RemoverCarrinho(int itemId)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                connection.Open();
                ...
                {
                    command.Connection = connection;
                    command.CommandText = "DELETE FROM tb_carrinhoCompras WHERE id_carrinhoCompra = @id";
